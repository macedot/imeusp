...
static double geraFuncao(double* pValores, double* pCoefA, double* pCoefU)
{
  register unsigned int nIdxP, nIdx;
  ...
  *pAtuA++ = geraVarA(X, Y);
  ...
  while(nIdxP < NumPts)
  {
    ...
    *pVal = *pAtuA;
    ...
    for(nIdx = 0; (nIdx < NumCos); nIdx++)
    {
      *pVal += (*pAtuA * cos(M_2PI * nIdx * (x + *pAtuU))) / base2;
      base2 *= 2.0;
      pAtuA++;
      pAtuU++;
    }
    for(nIdx = 0; (nIdx < NumSen); nIdx++)
    {
      *pVal += (*pAtuA * sin(M_2PI * nIdx * (x*x + *pAtuU))) / pow(2.0, (nIdx + fabs(*pAtuA)));
      pAtuA++;
      pAtuU++;
    }
